...
OUTPUT_ROOT="${OUTPUT_ROOT:-artifacts/checkpoints/ppo_expert_bc_loss_ablation}"
LOG_ROOT="${LOG_ROOT:-artifacts/logs/ppo_expert_bc_loss_ablation_runs/$(date +%Y%m%d_%H%M%S)}"
...
run_one() {
  ...
  echo "=== variant=${name} seed=${seed} log_std=${log_std_init} bc_kl=${bc_kl_coef} freeze=${freeze_actor_steps} expert_bc=${expert_bc_loss_coef} ==="
  echo "log=${run_log}"
  ...
  uv run --all-extras python scripts/fine_tune_ppo_from_bc.py \
    ...
    --expert-bc-loss-batch-size "${BATCH_SIZE}" 2>&1 | tee "${run_log}"
...
run_variant() {
  ...
  for seed in ${SEEDS}; do
    run_one "${name}" "${log_std_init}" "${bc_kl_coef}" "${freeze_actor_steps}" "${expert_bc_loss_coef}" "${seed}"
  done
}

run_variant expertbc_0p1_kl3e3_freeze25k -1.0 0.003 25000 0.1
run_variant expertbc_1p0_kl3e3_freeze25k -1.0 0.003 25000 1.0
